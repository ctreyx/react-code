 ...
 * @LastEditTime: 2024-09-11 14:12:29
 ...
 * 
 * 全程只有一个work,所以在有新任务 插入的时候，需要对比上次任务优先级和本次优先级，如果相同，则不执行，如果不同，则取消上次任务，插入新任务，执行新任务
 * 
 * 
 *
 */
...
[LowPriority, NormalPriority, UserBlockingPriority,ImmediatePrity].forEach(
	(it) => {
...
function perform(work, didTimeout) {
	...
	const needSync =
		work.priority === ImmediatePrity || didTimeout || shouldYield();

	// 如果是同步执行或时间切片还存在时间 并且存在任务，则执行

	while ((needSync || !shouldYield()) && work.count) {
		work.count--;
		insertSan(work.priority);
	}

	// 中断 ||  执行完，移除
	prevPriority = work.priority; // 保存上次执行的优先级，方便做策略逻辑
	if (!work.count) {
		const index = workLisk.indexOf(work);
...
function dosomeBuzy(len: number) {
	let result = 0;
	while (len--) {
		result += len;
	}
